; Modificar o código base enviado em anexo para entrar no modo protegido e printar na tela “hello protected mode!”
; Endereço base da memória de vídeo: 0xb8000
...
gdt_code:
	dw 0xFFFF		; Bits 0-15 of the segment limit
...
TEXT_CONF equ 0x2400		; 4bit bg color, 4bit text color, 8bit ascii (Green bg, Red text)
...
boot2:
	...
	mov gs, ax
	mov ss, ax
	mov esi, hello			; set string address to esi
	mov ebx, VIDEO_MEMORY	; set video memory address to ebx
...
.loop:
	...
	or al,al
	jz halt
	or eax, TEXT_CONF	; set text color and background color configuration
